...
PG_URL_NOKORI = os.getenv("NOKORI_DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/nokoriuz")
...
def sync_to_nokoriuz(clean_title: str, episode_number: int, studio: str, video_file_id: str, matched_code: Optional[int] = None) -> Optional[int]:
    """nokoriuz (@NokoriUzBot) bazasiga qo'shadi."""
    conn = None
    try:
        conn = psycopg2.connect(PG_URL_NOKORI, connect_timeout=5)
        cur = conn.cursor(cursor_factory=RealDictCursor)
        ...
        if not anime_id:
            ...
            if anime_row:
                ...
                cur.execute("""
                    INSERT INTO animes (title, original_title, description, genre, type, status, unique_id, studio, image, created_at, updated_at)
                    VALUES (%s, %s, %s, %s, 'TV', 'ONGOING', %s, %s, %s, NOW(), NOW())
                    RETURNING id, unique_id
                """, (clean_title, clean_title, f"{clean_title} o'zbek tilida", "Anime, Sarguzasht", unique_id, studio or "UzDub", "https://telegra.ph/file/default.jpg"))
                anime_id = cur.fetchone()["id"]
                logger.info(f"🎉 [NokoriUz] Yangi anime qo'shildi: {clean_title} (Kod: #{unique_id})")
        ...
        logger.error(f"[NokoriUz] Sinxronizatsiyada xatolik: {e}")
        if conn:
            conn.rollback()
            conn.close()
        return None
...
def sync_to_aniriouz(clean_title: str, episode_number: int, studio: str, video_file_id: str) -> Optional[int]:
    """aniriouz (TarjimaAnimelar) bazasiga ham parallel sinxronlashtiradi."""
    conn = None
    try:
...
def sync_episode_to_bot_database(
    ...
    """NokoriUz va AniRioUz bazalariga to'g'ri unikal kod bilan yozadi."""
    ...
    unique_code = sync_to_nokoriuz(clean_title, episode_number, studio, video_file_id, matched_unique_id)
    sync_to_aniriouz(clean_title, episode_number, studio, video_file_id)
